...
public class CelestialSkyRenderer extends IRenderHandler {
    ...
    public CelestialSkyRenderer(ISkyData data)
    {

        this.data = data;
        this.numStars = data.starAmount();

        GL11.glPushMatrix();
        GL11.glNewList(this.starGLCallList, GL11.GL_COMPILE);

        if(data.canSeeStars()) { this.renderStars(); }
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc)
    {
        ...
        int colorInt = Integer.parseInt(data.getSkyColor().substring(2), 16); // Remove "0x" and parse as integer
        ...
        var12 = data.getPrimaryPlanetSize();

        if(data.isOrbitingBody()) { drawPrimaryPlanet(var23, var12); }

        // HOME:
        var12 = data.getStarSize();
        GL11.glScalef(0.6F, 0.6F, 0.6F);
        float rot = data.getStarRotation();
    ...
    private void drawPrimaryPlanet(Tessellator var23, float var12) {
        ...
        FMLClientHandler.instance().getClient().renderEngine.bindTexture(data.getPrimaryPlanet().getIcon());
        var23.startDrawingQuads();
